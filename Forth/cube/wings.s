...
// QUAD words only if needed
.if QUAD == 1

@ -----------------------------------------------------------------------------
        Wortbirne Flag_visible, "quad-emit"
quad_emit:
        @ ( c -- ) Emit one character
// void QUAD_sendChar(char ch)
@ -----------------------------------------------------------------------------
	push	{lr}
	movs	r0, tos
	drop
...
@ -----------------------------------------------------------------------------
        Wortbirne Flag_visible, "quadpos!"
quad_get_pos:
        @ ( x -- ) Set QUAD position
...
@ -----------------------------------------------------------------------------
        Wortbirne Flag_visible, "quadshutdown"
quadshutdown:
        @ ( u -- ) status   0 shutdown, 1 normal operation
...
@ -----------------------------------------------------------------------------
        Wortbirne Flag_visible, "quadclr"
quadclr:
        @ ( -- )
...
@ -----------------------------------------------------------------------------
        Wortbirne Flag_visible, "quadpwm"
quadpwm:
        @ ( u -- ) select PWM (brightness) for the Quad emit
// void QUAD_dimm(int PWM)
@ -----------------------------------------------------------------------------
	push	{lr}
	movs	r0, tos		// font
	drop
...
@ -----------------------------------------------------------------------------
        Wortbirne Flag_visible, "quadblink"
quadblink:
        @ ( u -- ) Let the Quad blink
// void QUAD_blink(int status)
@ -----------------------------------------------------------------------------
	push	{lr}
	movs	r0, tos		// status
	drop
...
@ -----------------------------------------------------------------------------
        Wortbirne Flag_visible, "quadsegment!"
set_quadpixel:
        @ ( u1 u2 u3 -- )
...
get_quadpixel:
        @ ( u -- )
// int QUAD_getSegment((uint8_t column, uint8_t segment)
@ -----------------------------------------------------------------------------
	push	{lr}
	movs	r1, tos		// segment
	drop

...
// Global Variables
// ****************
const char BSP_Version[] = "  * Firmware Package STM32Cube F4 V1.27.1, USB-CDC (C) 2022 STMicroelectronics \n";
...
int BSP_getVref(void) {
	...
	osMutexRelease(Adc_MutexID);
	return __LL_ADC_CALC_VREFANALOG_VOLTAGE(value, ADC_RESOLUTION_12B);
}
...
int BSP_getVbat(void) {
	...
	osMutexRelease(Adc_MutexID);
	return __LL_ADC_CALC_DATA_TO_VOLTAGE(ref_voltage_mv, value, ADC_RESOLUTION_12B) ;
}
...
int BSP_getCpuTemperature(void) {
	...
	return __LL_ADC_CALC_TEMPERATURE(ref_voltage_mv, value, ADC_RESOLUTION_12B);
}
